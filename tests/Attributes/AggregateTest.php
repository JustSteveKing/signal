<?php

declare(strict_types=1);

namespace JustSteveKing\Signal\Tests\Attributes;

use JustSteveKing\Signal\Attributes\Aggregate;
use PHPUnit\Framework\Attributes\CoversClass;
use PHPUnit\Framework\Attributes\Test;
use PHPUnit\Framework\TestCase;

#[CoversClass(Aggregate::class)]
final class AggregateTest extends TestCase
{
    #[Test]
    public function it_stores_a_description(): void
    {
        $attribute = new Aggregate(description: 'Order aggregate root');

        $this->assertSame('Order aggregate root', $attribute->description);
    }

    #[Test]
    public function it_defaults_to_empty_tags(): void
    {
        $attribute = new Aggregate(description: 'Order aggregate root');

        $this->assertSame([], $attribute->tags);
    }

    #[Test]
    public function it_stores_tags(): void
    {
        $attribute = new Aggregate(description: 'Order aggregate root', tags: ['orders', 'ddd']);

        $this->assertSame(['orders', 'ddd'], $attribute->tags);
    }
}
