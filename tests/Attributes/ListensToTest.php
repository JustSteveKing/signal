<?php

declare(strict_types=1);

namespace JustSteveKing\Signal\Tests\Attributes;

use JustSteveKing\Signal\Attributes\ListensTo;
use PHPUnit\Framework\Attributes\CoversClass;
use PHPUnit\Framework\Attributes\Test;
use PHPUnit\Framework\TestCase;

#[CoversClass(ListensTo::class)]
final class ListensToTest extends TestCase
{
    #[Test]
    public function it_stores_an_event_name(): void
    {
        $attribute = new ListensTo(event: 'OrderCreated');

        $this->assertSame('OrderCreated', $attribute->event);
    }

    #[Test]
    public function it_defaults_to_empty_description(): void
    {
        $attribute = new ListensTo(event: 'OrderCreated');

        $this->assertSame('', $attribute->description);
    }

    #[Test]
    public function it_defaults_to_empty_tags(): void
    {
        $attribute = new ListensTo(event: 'OrderCreated');

        $this->assertSame([], $attribute->tags);
    }

    #[Test]
    public function it_stores_a_description(): void
    {
        $attribute = new ListensTo(event: 'OrderCreated', description: 'Fires when an order is placed.');

        $this->assertSame('Fires when an order is placed.', $attribute->description);
    }

    #[Test]
    public function it_stores_tags(): void
    {
        $attribute = new ListensTo(event: 'OrderCreated', tags: ['orders', 'async']);

        $this->assertSame(['orders', 'async'], $attribute->tags);
    }
}
